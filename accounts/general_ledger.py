...
# Service Layer Import - introduced to reduce tight coupling and improve modularity
...
def process_gl_map(gl_map, merge_entries=True, precision=None, from_repost=False):
	"""
	Process GL Map - Refactored to use Service Layer.
	
	This function now delegates to GeneralLedgerService.process_gl_map()
	to maintain separation of concerns and reduce tight coupling.
...
def distribute_gl_based_on_cost_center_allocation(gl_map, precision=None, from_repost=False):
	"""
	Distribute GL based on Cost Center Allocation - Refactored to use Service Layer.
	
	This function now delegates to GeneralLedgerService for improved modularity.
	Keeps backward compatibility for any external callers.
	"""
	return GeneralLedgerService._distribute_gl_based_on_cost_center_allocation(gl_map, precision, from_repost)


@request_cache
def get_cost_center_allocation_data(company, posting_date, cost_center):
	"""
	Get Cost Center Allocation Data - Refactored to use Service Layer.
	
	Delegates to GeneralLedgerService while maintaining request caching.
	Keeps backward compatibility for external callers.
	"""
	return GeneralLedgerService._get_cost_center_allocation_data(company, posting_date, cost_center)


def merge_similar_entries(gl_map, precision=None):
	"""
	Merge Similar Entries - Refactored to use Service Layer.
	
	Delegates to GeneralLedgerService for improved modularity.
	Keeps backward compatibility for any external callers.
	"""
	return GeneralLedgerService._merge_similar_entries(gl_map, precision)


# Backward compatibility functions - these now delegate to the service layer
# to maintain existing external API while using the new service architecture

def get_merge_properties(dimensions=None):
	"""
	Get Merge Properties - Delegates to Service Layer.
	
	Keeps backward compatibility for external callers.
	"""
	return GeneralLedgerService._get_merge_properties(dimensions)


def get_merge_key(entry, merge_properties):
	"""
	Get Merge Key - Delegates to Service Layer.
	
	Keeps backward compatibility for external callers.
	"""
	return GeneralLedgerService._get_merge_key(entry, merge_properties)


def check_if_in_list(gle, gl_map):
	"""
	Check if in List - Delegates to Service Layer.
	
	Keeps backward compatibility for external callers.
	"""
	return GeneralLedgerService._check_if_in_list(gle, gl_map)


def toggle_debit_credit_if_negative(gl_map):
	"""
	Toggle Debit Credit if Negative - Delegates to Service Layer.
	
	Keeps backward compatibility for external callers.
	"""
	return GeneralLedgerService._toggle_debit_credit_if_negative(gl_map)


# The following code sections were removed and moved to GeneralLedgerService:
# - Original merge_similar_entries implementation
# - Original get_merge_properties implementation
# - Original get_merge_key implementation  
# - Original check_if_in_list implementation
# - Original toggle_debit_credit_if_negative implementation
#
# This refactoring improves modularity while maintaining all existing behavior.
